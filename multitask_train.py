...
from utils import *      # Keep as is, using HARCNN_Multitask model (defined in utils.py)
import os
import json
...
alpha_value = args.alpha  # Using alpha from command line argument
dataset = args.dataset  # Using dataset from command line argument

# --- Paths for saving and loading train/test split ---
if dataset == 'mhealth':
    SPLIT_FILE = "data/mhealth_train_test_split.pkl"  # File name for saving/loading train/test split
    KFOLD_FILE = "data/mhealth_kfold_indices.pkl" # File name for saving/loading KFold indices
    num_postures = 13
    num_positions = 3
elif dataset == 'wisdm':
    SPLIT_FILE = "data/wisdm_train_test_split.pkl"  # File name for saving/loading train/test split
    KFOLD_FILE = "data/wisdm_kfold_indices.pkl" # File name for saving/loading KFold indices
    num_postures = 18
    num_positions = 2
elif dataset == 'har':
    SPLIT_FILE = "data/12train_test_split.pkl"
    KFOLD_FILE = "data/12kfold_indices.pkl" # File name for saving/loading KFold indices
    num_postures = 12
    num_positions = 3
else:
...
experiment_dir = os.path.join(results_dir, "multitask", f"seed_{SEED}_alpha_{alpha_value:.2f}") # Directory path
...
for fold, (train_index, val_index) in enumerate(kfold_indices):
    ...
    for epoch in tqdm(range(num_epochs), desc=f"Epochs (Fold {fold+1}, alpha={alpha_value:.2f})", leave=False):
        ...
        for inputs1, inputs2, targets1, targets2 in train_loader:
            optimizer.zero_grad()
            outputs1, outputs2 = model(inputs1) # f1, f2 no longer needed
        ...
        if val_acc1_fold_epoch > best_val_acc1_fold:
            ...
            best_val_acc2_fold = val_acc2_fold_epoch # Store corresponding task2 acc
    ...
    # ****** Plotting training and validation curves for the fold ******
    ...
    plot_task_curves_trainval( # Call new function
        ...
        train_loss_sdd=fold_curve_data['val_loss_task1'], # Note: sdd might be a misnomer if it's val_loss_ce
    ...
    plot_task_curves_trainval( # Call new function
        ...
        train_loss_sdd=fold_curve_data['val_loss_task2'], # Note: sdd might be a misnomer if it's val_loss_ce
...
for fold, best_model_state in enumerate(best_model_states):
    ...
    with torch.no_grad():
        for inputs1, inputs2, targets1, targets2 in test_loader:
            outputs1, outputs2 = final_model(inputs1)
            _, predicted1 = torch.max(outputs1, 1)
            _, predicted2 = torch.max(outputs2, 1)
